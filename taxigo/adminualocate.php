...
<body>
	...
	<img src="images/slide.jpg" alt="" class="this-bg hidden-xs">
		<div class="inner-block" style="margin-top:-40%;">

                            <div class="chit-chat-heading">

                                  <h1>Alocated Vehicles</h1>
                            </div>
                            <div class="table-responsive" style="width:97%;  margin-left:3%; overflow:scroll; height:500px; ">
							  ...
							  <tbody>
							  <?php
//SELECT `al_id`, `vid`, `driver_regid`, `status` FROM `vehicle_alocation` WHERE 1
$sql1="SELECT * FROM `vehicle_alocation` WHERE `status`='0'";	
$result=mysqli_query($con,$sql1);
while($fetch=mysqli_fetch_array($result)){
	$vid=$fetch["vid"];
	$rid=$fetch["driver_regid"];
	...
	$sql5="SELECT * FROM `registration` WHERE `reg_id`='$rid'";	
...
<tr>
<td><?php echo $fetch["al_id"];?></td>
<td><?php echo "<a href=adminunalovehicleview.php?vid=",$fetch['vid'],"&page=",$d,">$vid</a>" ?></td>
<td><img src="<?php echo $fetch4["vphoto"];?>" width="55" height="55"/></td>
<td><?php echo "<a href=adminunalodriverview.php?rid=",$fetch['driver_regid'],"&page=",$d,">$rid</a>" ?></td>
<td><img src="<?php echo $fetch5["photo"];?>" width="55" height="55"/></td>
<form method="post" action="unalocat.php">
<input type="hidden" name="vid" value="<?php echo $vid?>">
<input type="hidden" name="rid" value="<?php echo $rid?>">
<input type="hidden" name="alid" value="<?php echo $fetch["al_id"];?>">
<td><input type="submit" name="unalocate" value="remove" style="border-radius:5px; margin-left:-100%;"></td>
</tr>
</form>
<?php
}
