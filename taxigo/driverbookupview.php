...
$logid=$_SESSION['id'];
$rid=$_SESSION['rid'];
$sql1="SELECT * FROM `taxi_booking` where `from_datetime`>'$date'";
$result=mysqli_query($con,$sql1);
while($fetch=mysqli_fetch_array($result)){
	$vid=$fetch["vid"];
	$alid=$fetch["aloc_id"];
	$u_lid=$fetch["login_id"];
	
	$sql7="SELECT * FROM `login` WHERE `login_id`='$u_lid'";	
	...
	$vname=$fetch4['vname'];
	$d="driverbookupview.php";
		 ?>

<tr>
<td><?php echo $fetch["bk_id"];?></td>
<td><?php echo "<a href=driverbookvehview.php?vid=",$fetch['vid'],"&page=",$d,">$vid</a>" ?></td>
<td><?php echo "<a href=driverbookuserprof.php?rid=",$urid,"&page=",$d,">$urid</a>" ?></td>
