...
{
	$m=$_POST["vid"];
$sql3="SELECT * FROM `vehicle_reg` WHERE `valoc`='1' and `vid`='$m'";
...
echo "<script>if(confirm('car is active!!!!')){document.location.href='adminvehiclview.php'}else{document.location.href='adminhome.php'};</script>";

}
else
{
$sql2="UPDATE `vehicle_reg` SET `status`=1 WHERE `vid`=$m";
...
<body>
	...
	<div class="div4">
<div class="div1">
<h1 style="color:white; margin-left:-15%;">VEHICLE DETAlLS</h1>
<?php 
$tid=$_SESSION['id'];
//INSERT INTO `vehicle_reg`(`vid`, `vname`, `vregno`, `vphoto`, `vseatno`, `vmodel`, `vcolor`, `vrate`, `status`)
$sql="SELECT * FROM `vehicle_reg` WHERE `status`=0";
$result=mysqli_query($con,$sql);
$d='adminvehiclview.php';
$i=0;
...
<form name="myform" action="#" method="post">
    ...
    <input style="color:#f77d0a" type="hidden" name="vid" value="<?php echo $row['vid']?>"/>
     ...
     <td style="margin-top:3%; margin-left:5%; color:blue;"><?php echo "<a style='color:blue;' href=admin_caredit.php?rid=",$row['vid'],"&page=",$d,">Edit</a>" ?></td>
